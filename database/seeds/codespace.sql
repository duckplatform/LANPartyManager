...
INSERT INTO `users` (`last_name`, `first_name`, `username`, `email`, `password`, `is_admin`, `is_moderator`, `badge_token`)
...
SELECT
  ...
  '2026-04-10 09:00:00'
WHERE NOT EXISTS (
  SELECT 1 FROM `announcements` WHERE `title` = 'Bienvenue sur le LAN Party Manager'
);
...
SELECT
  ...
  '2026-04-15 18:30:00'
WHERE NOT EXISTS (
  SELECT 1 FROM `announcements` WHERE `title` = 'Programme du week-end'
);
...
SELECT
  ...
  '2026-04-20 12:00:00'
WHERE NOT EXISTS (
  SELECT 1 FROM `announcements` WHERE `title` = 'Preparation de la prochaine saison'
);
...
INSERT INTO `events` (`name`, `start_at`, `location`, `status`, `created_at`, `updated_at`)
SELECT 'LAN Spring Showdown', '2026-04-25 10:00:00', 'Salle Polyvalente - Lyon', 'in_progress', '2026-04-01 09:00:00', '2026-04-25 10:00:00'
WHERE NOT EXISTS (
  SELECT 1 FROM `events` WHERE `name` = 'LAN Spring Showdown'
);

INSERT INTO `events` (`name`, `start_at`, `location`, `status`, `created_at`, `updated_at`)
SELECT 'Retro Arena Weekend', '2026-12-31 14:00:00', 'Maison des Associations - Lille', 'planned', '2026-03-28 14:00:00', '2026-04-25 10:00:00'
WHERE NOT EXISTS (
  SELECT 1 FROM `events` WHERE `name` = 'Retro Arena Weekend' AND `start_at` = '2026-12-31 14:00:00'
);

UPDATE `events`
SET `status` = 'planned', `updated_at` = NOW()
WHERE `name` = 'Retro Arena Weekend' AND `status` <> 'planned';

INSERT INTO `events` (`name`, `start_at`, `location`, `status`, `created_at`, `updated_at`)
SELECT 'Finales Hiver 2025', '2025-12-14 09:30:00', 'Centre Numerique - Bordeaux', 'ended', '2025-11-20 08:00:00', '2025-12-14 23:00:00'
WHERE NOT EXISTS (
  SELECT 1 FROM `events` WHERE `name` = 'Finales Hiver 2025' AND `start_at` = '2025-12-14 09:30:00'
);
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
...
WHERE NOT EXISTS (
  SELECT 1 FROM `games` WHERE `name` = 'EA Sports FC 26' AND `console` = 'PS5'
);

INSERT INTO `games` (`name`, `console`, `match_type`)
SELECT 'Mario Kart 8 Deluxe', 'Nintendo Switch', '2v2'
WHERE NOT EXISTS (
  SELECT 1 FROM `games` WHERE `name` = 'Mario Kart 8 Deluxe' AND `console` = 'Nintendo Switch'
);

INSERT INTO `games` (`name`, `console`, `match_type`)
SELECT 'Street Fighter 6', 'PC', '1v1'
WHERE NOT EXISTS (
  SELECT 1 FROM `games` WHERE `name` = 'Street Fighter 6' AND `console` = 'PC'
);

INSERT INTO `games` (`name`, `console`, `match_type`)
SELECT 'Trackmania Turbo', 'PC', 'solo'
WHERE NOT EXISTS (
  SELECT 1 FROM `games` WHERE `name` = 'Trackmania Turbo' AND `console` = 'PC'
);
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `rooms` r WHERE r.event_id = e.id AND r.name = 'Zelda'
  );
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `rooms` r WHERE r.event_id = e.id AND r.name = 'Mario'
  );
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `rooms` r WHERE r.event_id = e.id AND r.name = 'Samus'
  );
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `rooms` r WHERE r.event_id = e.id AND r.name = 'Link'
  );
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `rooms` r WHERE r.event_id = e.id AND r.name = 'Kirby'
  );
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `rooms` r WHERE r.event_id = e.id AND r.name = 'Tifa'
  );
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `rooms` r WHERE r.event_id = e.id AND r.name = 'Fox'
  );
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `rooms` r WHERE r.event_id = e.id AND r.name = 'Cortex'
  );
...
INSERT INTO `battles` (`event_id`, `game_id`, `room_id`, `status`, `score`, `notes`, `created_at`, `updated_at`)
SELECT e.id, g.id, r.id, 'in_progress', NULL,
       'Rencontre de demonstration deja affectee a une salle active.',
       '2026-04-24 18:00:00', '2026-04-25 10:15:00'
FROM `events` e
JOIN `games` g ON g.name = 'Street Fighter 6' AND g.console = 'PC'
JOIN `rooms` r ON r.event_id = e.id AND r.name = 'Zelda'
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
...
FROM `events` e
JOIN `games` g ON g.name = 'Mario Kart 8 Deluxe' AND g.console = 'Nintendo Switch'
JOIN `rooms` r ON r.event_id = e.id AND r.name = 'Mario'
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `battles` b
...
SET b.status = 'in_progress', b.updated_at = NOW(),
...
INSERT INTO `battles` (`event_id`, `game_id`, `room_id`, `status`, `score`, `notes`, `created_at`, `updated_at`)
SELECT e.id, g.id, r.id, 'planned', NULL,
...
FROM `events` e
JOIN `games` g ON g.name = 'Mario Kart 8 Deluxe' AND g.console = 'Nintendo Switch'
JOIN `rooms` r ON r.event_id = e.id AND r.name = 'Kirby'
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `battles` b
...
FROM `events` e
JOIN `games` g ON g.name = 'Trackmania Turbo' AND g.console = 'PC'
JOIN `rooms` r ON r.event_id = e.id AND r.name = 'Fox'
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `battles` b
...
INSERT INTO `battles` (`event_id`, `game_id`, `room_id`, `status`, `score`, `notes`, `created_at`, `updated_at`)
SELECT e.id, g.id, r.id, 'ended', '2-1',
       'Duel 1v1 ended conserve pour verifier l''historique et les scores.',
...
WHERE e.name = 'LAN Spring Showdown'
  AND NOT EXISTS (
    SELECT 1 FROM `battles` b
    WHERE b.event_id = e.id AND b.game_id = g.id AND b.notes = 'Duel 1v1 ended conserve pour verifier l''historique et les scores.'
  );

INSERT INTO `battles` (`event_id`, `game_id`, `room_id`, `status`, `score`, `notes`, `created_at`, `updated_at`)
SELECT e.id, g.id, r.id, 'ended', '36-34',
...
FROM `battles` b
JOIN `users` u ON u.email = 'hugo.bernard@lanparty.local'
WHERE b.notes = 'Duel 1v1 ended conserve pour verifier l''historique et les scores.'
  AND NOT EXISTS (
    SELECT 1 FROM `battle_players` bp WHERE bp.battle_id = b.id AND bp.user_id = u.id
  );
...
-- SELECT g.match_type, b.status, COUNT(*) AS total
-- FROM battles b
-- JOIN games g ON g.id = b.game_id
-- JOIN events e ON e.id = b.event_id
-- WHERE e.name = 'LAN Spring Showdown'
-- GROUP BY g.match_type, b.status
-- ORDER BY g.match_type, FIELD(b.status,'in_progress','installation','planifie','queue','ended');
--
-- 2) Verifier les slots d'occupation par salle (active + planifie)
-- SELECT r.name AS room, r.match_type,
--        SUM(CASE WHEN b.status IN ('installation','in_progress') THEN 1 ELSE 0 END) AS active_slot,
--        SUM(CASE WHEN b.status = 'planifie' THEN 1 ELSE 0 END) AS planifie_slot,
--        SUM(CASE WHEN b.status = 'queue' THEN 1 ELSE 0 END) AS waiting_attached
-- FROM rooms r
-- LEFT JOIN battles b ON b.room_id = r.id
-- JOIN events e ON e.id = r.event_id
-- WHERE e.name = 'LAN Spring Showdown'
-- GROUP BY r.id, r.name, r.match_type
-- ORDER BY r.match_type, r.name;
--
-- 3) Verifier l'ordre FIFO des rencontres en attente
-- SELECT b.id, g.match_type, g.name AS game, b.created_at, b.notes
-- FROM battles b
-- JOIN games g ON g.id = b.game_id
-- JOIN events e ON e.id = b.event_id
-- WHERE e.name = 'LAN Spring Showdown' AND b.status = 'queue'
-- ORDER BY b.created_at ASC;
--
-- 4) Verifier les rencontres en attente bloquees par joueurs deja engages
-- SELECT bw.id AS waiting_id, g.name AS game, u.username AS joueur_bloquant,
--        bo.id AS battle_engagee, bo.status AS statut_engage,
...
-- WHERE e.name = 'LAN Spring Showdown'
--   AND bw.status = 'queue'
--   AND bo.status IN ('planifie','installation','in_progress')
-- ORDER BY bw.created_at, u.username;